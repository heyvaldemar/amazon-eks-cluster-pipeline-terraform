...
run "the_defaults_are_the_secure_ones" {
  ...
  assert {
    condition     = alltrue([for e in aws_eks_cluster.eks_cluster_1.encryption_config : contains(e.resources, "secrets")])
    error_message = "eks_cluster_1 stores Kubernetes secrets without envelope encryption"
  }
